...
//    private Button btn_take_photo, btn_pick_photo, btn_cancel;
    ...
    public MyPop(final Context mContext, final MyDetailBean.DatasBean.GoodsInfoBean goods_info, final MyApp myApp) {
        this.myApp = myApp;
        this.dbManager = myApp.getDbManager();
        this.goods_info = goods_info;
        this.view = LayoutInflater.from(mContext).inflate(R.layout.addshoppingpop_window, null);
        TextView addShopping = view.findViewById(R.id.addShopping);
...
//        Glide.with(mContext).load(goods_info.get)
        ...
        addShopping.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                shooping.setName(goods_info.getGoods_name());
                shooping.setCount(count);
                shooping.setPrice(goods_info.getGoods_price());
                List<Shooping> shoopings = dbManager.queryUserList();
                for (int i = 0; i < shoopings.size(); i++) {
                    Shooping shooping1 = shoopings.get(i);
                    if (shooping1.getName().equals(goods_info.getGoods_name())) {
                        Toast.makeText(mContext, "已经存在", Toast.LENGTH_SHORT).show();
                        dismiss();
                        return;
                    }
                }
                dbManager.insertUser(shooping);
